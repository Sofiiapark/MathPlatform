...
async function logError(err) {
    ...
    await promises_1.default.appendFile("errors.log", `[${new Date().toISOString()}] ${errorText}\n`);
}
async function consumeMessages() {
    try {
        ...
        console.log("[Consumer]  Waiting for messages from all platform modules...");
        channel.consume(q.queue, async (msg) => {
            ...
            try {
                ...
                // Зберігання у файл
                await promises_1.default.appendFile("messages.json", JSON.stringify(processed) + "\n");
                ...
                await logError(`Failed to process message with key ${routingKey}. Content: ${msg.content.toString()}. Error: ${err}`);
